...
struct rt_rvz_state {
    ...
    uint32_t stored_bytes;        /* size of stored_buffer (0: no group is compressed) */
    uint32_t group_bytes;         /* size of group_buffer */
    ...
    uint32_t on_usb;              /* 1: the RVZ is on the USB drive, read through the context's
                                   * usb_fd (d2x's /dev/usb2); 0: on the card (sdio_fd). The
                                   * group table is always on the card. */
    /* Runtime state. */
