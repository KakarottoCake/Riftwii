...
struct RvzResidentOptions {
    ...
    // The RVZ is on the USB drive: d2x's /dev/usb2 handle (wii/d2xusb.hpp)
...
struct ResidentOptions {
    ...
    // The crash note's card sector (sd:/riftwii/lastgame.txt, one sector)
    // and its first lines; 0 = none. Needs `sdio_fd`.
