...
| RiftWii channel boot program (design) | `channel/loader/` | FIX94's tiny vWii NAND Loader (https://github.com/FIX94/tiny-vwii-nand-loader, GPL-2.0), DacoTaco's OpenDolBoot (https://github.com/DacoTaco/priiloader, `tools/OpenDolBoot`, GPL-2.0), the Homebrew Channel's channel stub (https://github.com/fail0verflow/hbc, `wiipax/stub`, GPL-2.0), libogc's IPC layer, read as documentation | The two-stage layout (a small boot program with a section at `0x80003400` and a virtual entry, which reads the real program from another content) and the CPU set-up follow these; the code is RiftWii's own, no text copied. |
| M PLUS Rounded 1c (menu font) | `wii/font/rounded.ttf`, `wii/font/OFL.txt`; embedded brotli-compressed as `wii/assets/menufont.bin` | M+ Fonts Project, Bold weight (Google Fonts `ofl/mplusrounded1c/MPLUSRounded1c-Bold.ttf`), subset by `tools/make_menu_font.py` to Latin with accents, kana, JIS level-1 kanji and the characters of the translations and GameTDB's Japanese titles | SIL Open Font License 1.1 (`wii/font/OFL.txt`). The menu's artwork (tiles, buttons, pointer, bar) is drawn by RiftWii at start (`wii/skin.cpp`); it ships no image files. The Homebrew Channel icons (`hbc/icon.png`, `channel/installer/hbc/icon.png`) and the RiftWii channel's banner, icon, start screen and sound (`channel/art`, `channel/forwarder/data`) are drawn by `tools/make_channel_art.py` and `tools/make_channel.py` with the same font. |
...
| Zstandard (decoder) | `vendor-zstd/` (`zstddeclib.c`, `zstd.h`, `zstd_errors.h`) | 1.5.7, https://github.com/facebook/zstd; `zstddeclib.c` generated by its `build/single_file_libs/combine.py` | BSD-3-Clause (`vendor-zstd/LICENSE`; zstd is dual BSD/GPLv2, used under BSD) |
...
| GameCube controller adapter (WUP-028) | `runtime/rtgcad.*`, `runtime/pad/`, `wii/padhook.*`, `wii/gcadapter.*` | wup-028-bslug by Alex Chadwick 2017 (https://github.com/Chadderz121/wup-028-bslug), Dolphin's `GCAdapter.cpp` and IOS HID v4/v5 emulation, fakemote (embedded-game-controller), Nintendont's USB HID driver (FIX94 and contributors, https://github.com/FIX94/Nintendont, `kernel/HID.c`), wiibrew's /dev/usb/hid (v4), /dev/usb/hid (v5) and /dev/usb/ven pages | wup-028-bslug is MIT; Dolphin and fakemote are GPL-2.0-or-later, read as documentation. Nintendont is GPL-2.0; its developers allow RiftWii to use its code with attribution, given here. The adapter's init and rumble commands, its 37-byte report layout and the HID request sequences follow these; the driver and the PADRead/PADControlMotor hooks are RiftWii's own code, no text copied. |
...
- The RVZ reader (`src/rvz.cpp`, `runtime/rtrvz.c`) follows the format
