...
struct RvzRuntimeTable {
    ...
    std::uint32_t max_compressed = 0;  // the largest Zstandard-compressed group as stored (0: none)
    std::uint32_t max_raw = 0;       // the largest group stored as it is
...
// The in-game runtime's buffers for a table (32-byte multiples): `stored`
// takes a compressed group as stored (0 when no group is compressed, and
// then no decoder is needed either), `group` a group decoded or stored as
// it is. Each is as large as the table needs, plus the whole sectors a
// fetch rounds out to.
struct RvzBufferSizes {
    std::uint32_t stored = 0;
    std::uint32_t group = 0;
    bool decoder = false;
};
// Hash exceptions a group's lists may hold before its decoded form
// outgrows the buffer: a disc as pressed has none.
constexpr std::uint32_t kRvzExceptionAllowance = 400;
RvzBufferSizes rvz_buffer_sizes(const RvzRuntimeTable& table);
