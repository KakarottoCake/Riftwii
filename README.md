...
`<source>:/games`. Dolphin's compressed `.rvz` images play from
`<source>:/games` too (the SD card must be in the Wii even for an RVZ on
USB). RiftWii checks each RVZ before it plays: the kinds it reads well
play, heavier compression plays after a warning, and what it cannot read
is refused with the reason. See `docs/RVZ.md`.
