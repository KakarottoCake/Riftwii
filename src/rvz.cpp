    ...
    for (std::size_t s = 0; s < 2; ++s) {
        ...
        for (std::uint32_t j = 0; j < needed; ++j) {
            ...
            if (word1 & 0x80000000u) {
                out.max_compressed = std::max(out.max_compressed, stored);
            } else {
                out.max_raw = std::max(out.max_raw, stored);
            }
...
RvzBufferSizes rvz_buffer_sizes(const RvzRuntimeTable& table) {
    const auto align = [](std::uint32_t n) { return (n + 31u) & ~31u; };
    constexpr std::uint32_t kRounding = 2 * 512;  // a fetch starts mid-sector and ends on a whole one
    RvzBufferSizes out;
    std::uint32_t group = table.max_raw != 0 ? table.max_raw + kRounding : 0;
    if (table.max_compressed != 0) {
        out.stored = align(table.max_compressed + kRounding);
        out.decoder = true;
        const std::uint32_t decoded = std::max(table.max_body, table.group_kib * 1024u) +
                                      table.lists * (2 + 22 * kRvzExceptionAllowance) + 64;
        group = std::max(group, decoded);
    }
    out.group = align(group);
    return out;
}
