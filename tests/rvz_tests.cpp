...
void CheckRuntime(const char* name, bool d2x, std::uint32_t piece, bool usb = false) {
    std::unique_ptr<RvzImage> image;
    std::string error;
    ...
    const std::uint32_t decoded = std::max(table.max_body, table.group_kib * 1024u) + table.lists * (2 + 22 * 400) + 64;
    const std::uint32_t buffer_bytes = (std::max(decoded, table.max_stored + 1024u) + 31) & ~31u;
    const std::uint32_t dctx_bytes = 0x40000;
    const std::size_t low_bytes = 0x10000 + sizeof(rt_rvz_state) + 2 * std::size_t(buffer_bytes) + dctx_bytes + 0x60000;
    ...
    std::uint8_t* group = stored + buffer_bytes;
    std::uint8_t* dctx = group + buffer_bytes;
    ...
    st.usb_fd = usb ? kUsbFd : 0xFFFFFFFFu;
    ...
    EXPECT_EQ(g_usb_reads != 0, usb);  // the groups come from the drive holding the RVZ
