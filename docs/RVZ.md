...
2. **Menu and loader (done; Dolphin-tested).** `.rvz` files in
   ...
   both own the drive. `di::read` answers partition
...
3. **In-game reads (done; Dolphin-tested, not yet on a Wii).** RVZ games
   ...
   the RVZ: 735 reads, 365 groups decoded, none failed.
...
stored, one for it decoded, and the decoder's workspace: about 370 KB
for 128 KiB chunks.
...
still applies on top (MEM, SD and relocated files; not yet tried with a
mod), and what it leaves to the disc comes from the RVZ. A read past the partition's data fails as it
...
request as d2x's SD read), on the handle the loader opened: d2x refuses
to open the device once a title runs. The group table and the request
